...
`scripts/Dockerfile.ubuntu20` **валят сборку**, если vcpkg поставил версию ниже. Какая версия реально
попала в бинарь, видно из 1С методом `ВерсияСборки` (`GetBuildInfo`).
...
дополнительных мер — проверено сборкой без этой опции: `Exception.hh` подключает `<fmt/core.h>`, а тот
тянет `format.h`. Но на других сочетаниях версий в цепочку попадает только базовый заголовок fmt, и сборка
падает на `'format' is not a member of 'fmt'` — мы это ловили. Поэтому в `CMakeLists.txt` для каждого TU
добавлено `/FI fmt/format.h` (MSVC) и `-include fmt/format.h` (GCC/Clang): страховка от того, какой именно
заголовок подтянет конкретная версия avro-cpp, и она ничего не ломает, когда проблемы нет.
...
Подключение в 1С — [connection.md](connection.md). После обновления компоненты стоит вызвать
`ВерсияСборки()` и убедиться, что загрузилась ожидаемая версия: 1С кэширует внешние компоненты, и имя
файла в макете может врать.

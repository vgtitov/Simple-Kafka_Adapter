...
> обнуляет счётчик: `1.9.1-fork.2` → (апстрим 1.9.2) → `1.9.2-fork.1`.

## [1.9.2-fork.2] - 2026-08-12

Ответ на решение автора апстрима по вопросу
[issue #87](https://github.com/NuclearAPK/Simple-Kafka_Adapter/issues/87): вместо угадывания base64
на входе декода — явный параметр, которым управляет вызывающий.
...
### Открыто
- Апстрим выпустил `1.9.3` (влил PR #82–#86, только версия/доки/CHANGELOG сверху). Синхронизация базы
  форка с `1.9.3` — отдельная задача (`docs/UPSTREAM.md`), не блокирует этот релиз: код за счёт наших
  топик-веток по факту уже содержит все пять фиксов из `1.9.3`.

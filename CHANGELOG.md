...
> Версии с суффиксом `-fork.N` — это сборки форка поверх соответствующей апстрим-версии
> (`X.Y.Z`). Содержат изменения, которых пока нет в апстриме (NuclearAPK).

## [1.9.1-fork.3] - 2026-07-27

Исправление аварийного завершения компоненты при **отправке** AVRO (регресс, внесённый в `1.9.1-fork.2`).
...
## [1.9.1-fork.1] - 2026-06-23

Форк поверх апстрим **1.9.1**. Тема релиза — проброс реального текста ошибок librdkafka в 1С через
`GetLastError` / `ПолучитьСообщениеОбОшибке`. Раньше в ряде сценариев операция завершалась ошибкой, но
1С получала `Ложь`/пустой результат без причины, либо причина оседала только в лог-файле.
...
- Асинхронные ошибки librdkafka (сбой SASL-аутентификации, недоступный брокер, ошибка SSL-handshake,
  сбой доставки) приходят в фоновом потоке через `event_cb`/`dr_cb`. Раньше они писались только в лог и при
  пустом `КаталогЛогов` терялись полностью. Добавлен потокобезопасный слот последней асинхронной ошибки;
  `GetLastError` возвращает её, если синхронной ошибки нет. Слот сбрасывается при инициализации
  продюсера/консьюмера, чтобы старая ошибка не маскировала успешное переподключение.
